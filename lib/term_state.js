...
var deepEqual = myUtil.deepEqual;
...
TermState.prototype._createLine = function(line) {
	...
	var lineWidth = getWidth(this._modes.stringWidth, line.str);
	for(var i in line.attr) {
		if(isNaN(i))
			continue;
		if(i > lineWidth || line.attr[i] === undefined)
			delete line.attr[i];
	}
	return line;
};
...
TermState.prototype._lineInject = function(content, wrapped) {
	...
	this._applyAttributes(line, c.x, getWidth(stringWidth, content));
	if(wrapped)
...
TermState.prototype.removeChar = function(count) {
	...
	var stringWidth = this._modes.stringWidth;
	var lineWidth = getWidth(stringWidth, line.str);
	var last = line.attr[lineWidth];
	myUtil.objSplice(line.attr, lineWidth+1, c.x, count);
	line.str = substrWidth(stringWidth, line.str, 0, c.x) +
		substrWidth(stringWidth, line.str, c.x+count);
	line.attr[lineWidth] = last;
	this.setLine(c.y, line);
};
...
TermState.prototype.insertBlank = function(count) {
	...
	myUtil.objSplice(line.attr, lineWidth+1, c.x, 0, new Array(count));
	myUtil.objSplice(line.attr, lineWidth+1, this.columns);
	line.str = substrWidth(stringWidth, line.str, 0, c.x) +
		myUtil.repeat(" ", count) + substrWidth(stringWidth, line.str, c.x);
	line.str = substrWidth(stringWidth, line.str, 0, this.columns);
	line.attr[getWidth(stringWidth, line.str)] = last;
	this.setLine(c.y, line);
};
...
TermState.prototype.setLine = function(nbr, line) {
	...
	if(this._buffer.str.length <= nbr) {
		...
		var stringWidth = this._modes.stringWidth;
		var lineWidth = getWidth(stringWidth, line.str);
		if(lineWidth > this.columns)
			line.attr[this.columns] = line.attr[lineWidth];
		this._buffer.str[nbr] = substrWidth(stringWidth, line.str, 0, this.columns);
...
TermState.prototype._applyAttributes = function(line, index, len) {
	var li, last, pre, now, pi, i;

	for(li = index+len; li > 0 && line.attr[li] === undefined; li--);
	last = line.attr[li];

	for(pi = index-1; pi > 0 && line.attr[pi] === undefined; pi--);
	pre = now = line.attr[pi];

	for(i = index; i < index+len; i++)
		delete line.attr[i];

	var lineWidth = getWidth(this._modes.stringWidth, line.str);
	if(!deepEqual(pre, this._attributes))
		line.attr[index] = now = this._attributes;
	if(index + len <= this.columns && !deepEqual(now, last))
...
TermState.prototype.eraseCharacters = function(count) {
	...
	line.str = substrWidth(stringWidth, line.str, 0, c.x) +
		myUtil.repeat(" ", count) + substrWidth(stringWidth, line.str, c.x+count);
